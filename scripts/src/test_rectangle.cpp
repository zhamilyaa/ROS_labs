...
int main(int argc, char **argv)
{
  ...
  static const std::string PLANNING_GROUP = "base"; /* Now we specify with what group we want work,
  ...
  target_pose.pose.position.x = target_pose.pose.position.x - 0.5; /* Basically our target pose is the same as current,
  ...
  std::cout << target_pose;
    ...
    if (abs(current_pose.pose.position.x - target_pose.pose.position.x) < 0.01){
		target_pose.pose.position.y = target_pose.pose.position.y - 1.0;
		move_group.setApproximateJointValueTarget(target_pose); // To calculate the trajectory
		move_group.move(); // Move the robot
		current_pose = move_group.getCurrentPose();
		std::cout << target_pose;
		if (abs(current_pose.pose.position.y - target_pose.pose.position.y) < 0.01){
			target_pose.pose.position.x = target_pose.pose.position.x + 0.2;
			move_group.setApproximateJointValueTarget(target_pose); // To calculate the trajectory
			move_group.move(); // Move the robot
			current_pose = move_group.getCurrentPose();
			std::cout << target_pose;
